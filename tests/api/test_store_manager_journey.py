...
class TestStoreManagerJourney:
    ...
    def test_01_manager_adds_new_pet_to_inventory(self, api_client, logger):
        ...
        assert_field_value(
            created_pet, "name", pet_data["name"], "Pet name should match"
        )
        assert_field_value(
            created_pet, "status", "available", "New pet should be available"
        )
    ...
    def test_02_manager_updates_pet_information(self, api_client, logger):
        ...
        assert_field_value(
            updated_pet, "status", "pending", "Pet status should be updated"
        )
    ...
    def test_06_manager_verifies_sold_pet_not_available(self, api_client, logger):
        ...
        assert (
            sold_pet_id not in available_pet_ids
        ), "Sold pet should not be in available list"
